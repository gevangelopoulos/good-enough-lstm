function [results]= forward_pass_piecewise_sigmf(mode,xdata,ydata)
startup;
dsSize=80;
fce_acc=0;
defaults= [1 0];
...
for seqNum=1:dsSize
    ...
    %Forward    
        ...
        F_in_gate=piecewise_sigmf(F_n1,defaults,mode,xdata,ydata);
        F_in_transform=tansig(F_n2);
        F_forget_gate=piecewise_sigmf(F_n3,defaults,mode,xdata,ydata);
        F_out_gate=piecewise_sigmf(F_n4,defaults,mode,xdata,ydata);
    ...
    B_in_gate=piecewise_sigmf(B_n1,defaults,mode,xdata,ydata);
    B_in_transform=tansig(B_n2);
    B_forget_gate=piecewise_sigmf(B_n3,defaults,mode,xdata,ydata);
    B_out_gate=piecewise_sigmf(B_n4,defaults,mode,xdata,ydata);
